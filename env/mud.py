"""
This is the environment file for MUD.
- Legal decision making: multi-defendant charge prediction.
[Ref] Through the MUD: A Multi-Defendant Charge Prediction Benchmark with Linked Crime Elements
"""
import ast
import json
from .base import Env
from .prompts.mud_prompt import ZERO_SHOT_PROMPT, CASE_PROMPT, CBR_PROMPT, CASE_PROMPT_REFLEXION
...
class MUDEnv(Env):
    ...
    def init_env(self):
        with open("data/mud/mud.jsonl", encoding="utf-8") as file:
            for line in file:
    ...
    def extraction(self, generated_text):
        try:
            answer = generated_text.split('```json')[-1].split('```')[0]
            ans_dict = ast.literal_eval(answer)
        except Exception:
            ans_dict = None
        return ans_dict
        ...
        if generated_answer is None:
            ...
            try:
                ...
                for key in ground_truth:
                    if ground_truth[key] == generated_answer[key]:
                        count += 1
                reward = int(count == len(ground_truth))
            except Exception:
                reward = 0
        return reward
