"""
This is the environment file for MIMIC-IV-MSR.
- Clinical decision making: specialty recommendation.
[Ref] Evaluating large language model workflows in clinical decision support for triage and referral and diagnosis
"""
import ast
import json
from .base import Env
from .prompts.mimic_iv_msr_prompt import ZERO_SHOT_PROMPT, CASE_PROMPT, CBR_PROMPT, CASE_PROMPT_REFLEXION

class MIMIC_IV_MSREnv(Env):
    def __init__(self):
        super().__init__()
        self.dataset = []
        self.init_env()
    ...
    def init_env(self):
        with open("data/mimic-iv-msr/mimic-iv-msr.jsonl", encoding="utf-8") as file:
            for line in file:
    ...
    def reward_function(self, generated_answer):
        if generated_answer is None:
            ...
            reward = int(generated_answer in ground_truth)
        return reward
