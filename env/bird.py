"""
This is the environment file for BIRD.
- Code generation: text-to-SQL.
[Ref.1] Can LLM Already Serve as A Database Interface? A BIg Bench for Large-Scale Database Grounded Text-to-SQLs
...
from .prompts.bird_prompt import ZERO_SHOT_PROMPT, CASE_PROMPT, CASE_PROMPT_REFLEXION, CBR_PROMPT, PROBLEM_WITH_EVIDENCE_PROMPT
...
class BIRDEnv(Env):
    def __init__(self):
        super().__init__()
        self.dataset = []
        self.db_path = "data/bird/source/dev_databases"
        self.init_env()
        ...
        self.PROBLEM_WITH_EVIDENCE_PROMPT = PROBLEM_WITH_EVIDENCE_PROMPT
        
    def init_env(self):
        # Load dataset
        with open("data/bird/bird.jsonl", encoding="utf-8") as file:
            for line in file:
    ...
    def observe(self):
        ...
        evidence = self.dataset[self.index]["evidence"]
        if evidence:
            problem = self.PROBLEM_WITH_EVIDENCE_PROMPT.format(problem=problem, evidence=evidence)
        return problem
    ...
    def reward_function(self, generated_answer):
        if generated_answer is None:
            ...
            ground_truth_script = self.dataset[self.index]["SQL"]
            db_id = self.dataset[self.index]["db_id"]
    ...
    def get_ground_truth(self):
        return self.dataset[self.index]["SQL"]